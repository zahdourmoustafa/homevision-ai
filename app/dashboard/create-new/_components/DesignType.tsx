      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        {Designs.map((design) => (
          <div
            ...
            className={`flex flex-col items-center p-2 border rounded-lg space-y-1 cursor-pointer transition-colors duration-200
              ${selected === design.name ? 'border-blue-500 bg-blue-100 hover:bg-blue-200' : 'border-gray-200 hover:border-blue-500 hover:bg-blue-100'}`}
          >
            ...
            <span className={`text-xs font-bold ${selected === design.name ? 'text-blue-600' : 'text-gray-900'}`}>
              {design.name}
            </span>
          </div>
        ))}
